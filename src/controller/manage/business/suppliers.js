...
export default class extends Base {
    ...
    async getAllAction() {
        ...
        if (param.keyword && param.keyword != '') {
            if (!(/^1[3|4|5|8][0-9]\d{0,8}$/.test(param.keyword))) {
                supplierList = await this.model('suppliers')
            ...
            supplierList = await this.model('suppliers').page(param.pageNum, param.pageSize).countSelect();
        }
    ...
    async createAction() {
        ...
        let insertId = await this.model('suppliers').add(param);
    ...
    async deleteAction() {
        const param = this.ctx.post();
        let model = this.model('suppliers');
        try {
    ...
    async findByIdAction() {
        const param = this.ctx.param();
        let detail = await this.model('suppliers').where({ id: ['=', param.id] }).find();
    ...
    async updateAction() {
        const param = this.ctx.post();
        let affectRows = await this.model('suppliers').where({ id: param.id }).update(param);
        return this.success();
    }
    ...
    async exportAction() {
        try {
            ...
            let data = await this.model('suppliers').select();
            let xlsxData = [];
    ...
    async importAction() {
        ...
        const model = this.model('suppliers');
        try {
            await model.startTrans();
